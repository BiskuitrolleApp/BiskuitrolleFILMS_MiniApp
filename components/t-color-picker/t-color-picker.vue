...
export default {
	...
	methods: {
		...
		changeColorType(data) {
			...
			if (rgb.length == 3) {
				let colorGroup = this.group(rgb, 1);
				...
				let colorGroup = this.group(rgb, 2);
				color.r = parseInt(colorGroup[0], 16);
				color.g = parseInt(colorGroup[1], 16);
				color.b = parseInt(colorGroup[2], 16);
				if (colorGroup[3] && colorGroup[3] !== 'ff') {
					let a = parseInt(colorGroup[3], 16);
					color.a = tools.formatDecimal(a / 255, 1);
				} else {
					let a = parseInt('ff', 16);
					color.a = 1;
				}
			}
			return color;
